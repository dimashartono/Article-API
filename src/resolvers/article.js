...
module.exports = {
	Query: {
		async showAllArticle(root, { options }, ctx) {
			try {
				...
				let getArticles = await article.aggregate([{ $sort: { createdAt: sort } }, { $limit: limit }, { $skip: offset }])
		...
		async showArticleById(root, { id }, ctx) {
			try {
	...
	Mutation: {
		async addArticle(root, { input }, ctx) {
			try {
